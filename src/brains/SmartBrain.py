    ...
    def placeArmy(self, armyHeight):
       ...
       ####################################################################################################################################
        ...
        ####################################################################################################################################
            ...
            for column in range(BOARD_WIDTH):
                if positions.index((column, frontrow)) > 0 and self.game.isWater(column, frontrow + direction):
                    posFlagCol += [column]
        ...
        for column in range(BOARD_WIDTH):
            if positions.__contains__((column, frontrow)) and not self.game.isWater(column, frontrow + direction):
                bool = randint(0,1)

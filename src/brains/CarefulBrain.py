...
# (CDLTLL) import randomBrain

from brains.randomBrain import Brain as RandomBrain
...
# (CDLTLL)class Brain(randomBrain.Brain):
class Brain(RandomBrain):
    ...
    def placeArmy(self, armyHeight):
        ...
        if self.army.color == "Blue":
            ...
            self.rows = range(int(self.armyHeight))
        else:
        ...
        r = RandomBrain(self.game, self.army)

        # (CDLTLL)
        # r.placeArmy(self.armyHeight)
        r.placeArmy(int(self.armyHeight))
    ...
    def placeFlag(self):
        ...
        for n, i in enumerate(self.rows):
            ...
            for _ in range(int(self.armyHeight - n - 1)): # weighted toward back rows 
    ...
    def placeBombs(self, bombs):
        ...
        for row in range(int(min(self.frontrow, self.backrow)), int(max(self.frontrow, self.backrow)) + 1):
    ...
    def placeScouts(self, number=2):
        ...
        while placed < number and attempts < 1000:
            attempts += 1
            x = choice(columns)
            if not self.game.getUnit(x, self.frontrow) and not self.game.isWaterColumn(x):
